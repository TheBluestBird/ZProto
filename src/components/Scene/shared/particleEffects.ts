...
import type { TimeOfDay } from '@game/ambient/timeOfDay';

export type RoomKey = 'kitchen' | 'carpentry' | 'blacksmith';

export function ambientEffect(phase: TimeOfDay): SpawnConfig {
  switch (phase) {
    case 'morning':
      return {
        spawnPerSecond: 6,
...
export function roomDustEffect(): SpawnConfig {
  return {
    spawnPerSecond: 3,
    maxParticles: 18,
    init: (s, { bounds }) => {
...
export function roomWorkEffect(room: RoomKey): SpawnConfig {
  switch (room) {
    case 'kitchen':
      return {
        spawnPerSecond: 10,
